...
function draw() {
	...
	canvas.addEventListener("click", () => {
			...
			if (lavaHeight < 1) {
				...
				}
				{
					...
					speedRange.addEventListener("input", (e) => {
						speed = (inverted?-1:1) * 10 ** speedRange.value
						speedDisplay.innerHTML = Math.round(speed * 100) / 100
					})
					let invertBox = document.getElementById("inverted")
					invertBox.addEventListener("input", (e) => {
						inverted = invertBox.checked
						speedRange.dispatchEvent(new InputEvent("input"))
					})
				}
				...
				function addBomb() {
					bombs.push(new VolcanicBomb(speed))
					setTimeout(addBomb, 200 / Math.abs(speed))
				}
		...
		function drawGas() {
			if (time < 1.8) {
				period = Math.max(1, 1 / Math.abs(speed))
				if (++attempts >= period) {
					attempts -= period
					ashes.push(new Ash(speed))
				}
...
class VolcanicBomb {
	constructor(speed = 1) {
		...
		this.y = 0;
		if (speed < 0) {
			while (this.y < hm(700) && Math.abs(this.x) < wm(500)) {
				this.x += this.vx;
				this.y += this.vy;
				this.vy += hm(4);
			}
